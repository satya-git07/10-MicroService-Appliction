pipeline {
    ...
    stages {
        ...
        stage('SonarQube Analysis') {
            steps {
                script {
                    // Run SonarQube analysis for the repository or each microservice
                    sh """
        ...
        stage("Trivy Vulnerability Scan") {
            steps {
                script {
                    // Run Trivy vulnerability scan for each microservice
                    MICRO_SERVICES.each { service ->
        ...
        stage('Deploy to GKE') {
            steps {
                // Authenticate and deploy each microservice to GKE
                withCredentials([file(credentialsId: 'gcp-sa', variable: 'GOOGLE_APPLICATION_CREDENTIALS')]) {
                    script {
